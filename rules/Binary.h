...
#define Binary::writeFloat(data) (ENDIANNESS === 0 ? pack("f", data) : pack("f", strrev(data)))

#define Binary::readLFloat(data) (ENDIANNESS === 0 ? unpack("f", strrev(data))[1] : unpack("f", data)[1])
#define Binary::writeLFloat(data) (ENDIANNESS === 0 ? pack("f", strrev(data)) : pack("f", data))

#define Binary::readDouble(data) (ENDIANNESS === 0 ? unpack("d", data)[1] : unpack("d", strrev(data))[1])
#define Binary::writeDouble(data) (ENDIANNESS === 0 ? pack("d", data) : pack("d", strrev(data)))

#define Binary::readLDouble(data) (ENDIANNESS === 0 ? unpack("d", strrev(data))[1] : unpack("d", data)[1])
#define Binary::writeLDouble(data) (ENDIANNESS === 0 ? pack("d", strrev(data)) : pack("d", data))